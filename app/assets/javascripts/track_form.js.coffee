class @TrackForm
  @removeTrack: (el) ->
    ...
    if trackFormGroup.hasClass('new-record')
      ...
      inlineList.find('.edit-track').show().addClass('line-through')
  ...
  @doneTrack: (el) ->
    ...
    inlineList.find('.track-name').show().text(text)
    inlineList.find('.done-track').show()
  ...
  @restoreTrack: (el) ->
    ...
    inlineList = el.closest('li').siblings()
    el.hide()
    inlineList.find('.edit-track').show().removeClass('line-through')
    inlineList.find('.remove-track').show()
